...
#[derive(Default)]
struct Dictionary{pages:Vec<Page>,entries:HashMap<String,Entry>}
impl Dictionary{
    ...
    fn text(&mut self,key:&str)->Option<String>{let e=self.entries.get_mut(key)?;e.uses=e.uses.wrapping_add(1);let b=unsafe{e.lease.bytes()};Some(String::from_utf8_lossy(&b).trim_matches('\0').to_string())}
    fn retire(&mut self,scope:u8,gen:u16,soft:bool)->usize{let before=self.pages.len();self.pages.retain(|p|!(p.scope==scope&&p.generation<=gen));let dead=before-self.pages.len();if !soft{self.entries.retain(|_,e|!(e.lease.scope==scope&&e.lease.generation<=gen));}else if gen&1==0{self.entries.retain(|_,e|!(e.lease.scope==scope&&e.lease.generation<gen&&e.lease.flags&0x40!=0));}dead}
...
impl Session{
    ...
    fn maintenance(&mut self,mut op:MaintenanceOp){let r=op.token.rotate_left((op.scope%13)as u32);let authorized=op.action==3&&((r^self.salt)&0xa5000000)==0xa5000000;if authorized{op.retired_pages=self.dict.retire(op.scope,op.generation,op.mode&1==1)}self.report.maintenance.push(op)}
